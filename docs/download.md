---
# https://vitepress.dev/reference/default-theme-home-page
layout: home

hero:
  name: "Download mybatop"

features:
  - title: Debian
    details: Click to Download for Ubuntu/Debian (deb)
    link: https://github.com/vishnumur777/mybatop/releases/download/1.0.0/mybatop-1.0.0.deb
    #icon: <put svg here>
  - title: Arch Linux
    details: Click to Download for Arch Linux (yay)
    link: /docs/install#_3-arch-linux
    #icon: <put svg here>
  - title: Fedora
    details: Click to Download for Fedora/OpenSUSE/CentOS (rpm)
    link: https://github.com/vishnumur777/mybatop/releases/download/1.0.0/mybatop-1.0.0-1.fc41.noarch.rpm
    #icon: <put svg here>
  - title: Source Code Tarball
    details: Click to Download Source Code
    link: https://github.com/vishnumur777/mybatop/releases/download/1.0.0/mybatop-1.0.0.tar.gz
    #icon: <put svg here>
---